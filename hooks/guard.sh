...
# front of `ticket take`. A session may draft one and propose it; running the
# approval for itself would make the agreement a formality it grants itself.
# The prefix is what a user's explicit yes looks like at this layer.
if printf '%s' "$cmd" | grep -qE '(^|[^[:alnum:]_.-])devflow[[:space:]]+contract[[:space:]]+approve([[:space:]]|$)'; then
  echo "devflow guard: approving a contract is the user's act, not a session's. Put the drafted contract to the user and let them answer. If the user has said yes to this ticket, re-run prefixed with DEVFLOW_ALLOW=1." >&2
  exit 2
fi
