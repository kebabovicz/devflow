...
echo "devflow stop-gate: $(basename "$ticket") is in progress and the working tree is dirty — an iteration implemented work without committing it. The way out is the same either way: tick the acceptance boxes that are true, commit the ticket's files with a message referencing it, then hand it over with 'devflow ticket report $(basename "$ticket" .md) --commit <sha>'. Never commit on your own initiative when a person is waiting on the verdict — ask first, then report the commit they approved. A green test with a dirty tree and no one reviewing is a failed iteration." >&2
exit 2
