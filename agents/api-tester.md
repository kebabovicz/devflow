...
2. **Auth**: if any step needs it, obtain a token per the auth recipe — `auth.recipe` for service-level access, `auth.user_recipe` when the scenario acts as a specific user. The recipe in the manifest is the ONLY path you try: cannot obtain a token → report exactly which step failed; do not improvise bypasses or explore alternative grants. Empty recipe carrying a `# none` marker = confirmed public surface, proceed without auth; empty recipe WITHOUT the marker + a step answering 401/403 = manifest gap — report it as the finding, don't improvise.
