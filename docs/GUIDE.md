...
| `/devflow:discuss <idea>` | Pre-implementation interview: rounds on real forks only, alternating two instruments — option menus for enumerable forks, free-text dialogue that digs into answers — plus contentious calls argued with trade-offs and an explicit "unless you say otherwise" assumption ledger → an approved spec/concept file. Two grounds: a feature in an existing repo (axes: scope/non-goals, edge behavior, data contracts, integration, quality bar) or a greenfield idea with no repo yet (axes: audience and pain, core loop, MVP cut, platform/stack, data, distribution). "I don't know" gets proposed options → narrowing follow-ups → an honest open question + `/devflow:research`, never a silent pick. Never implements | The thing exists as a picture in your head — a feature or a whole new product — and the gap between "как я это представлял" and what gets built is expensive. Output feeds `task` (small), `ralph-plan` (large), or `init`+`ralph-plan` (greenfield) |
...
| `/devflow:review [branch \| PR]` | Iterative review: a pass judged by the project's own rules → fix accepted findings → re-review, until a clean pass (max 3 rounds, anti-churn rules). `swarm` runs the review pass as parallel per-dimension finders + adversarial verifiers (local Workflow, opt-in) — distinct from the cloud-billed `/code-review ultra` | "Polish until 10/10". A branch after a Ralph run; for a large risky diff prefer `/code-review ultra` or `review swarm` |
...
| The requirements live in your head, not in a ticket — a feature or a brand-new project | `/devflow:discuss` first — agree on the picture, then `task` / `ralph-plan` (or `init` for a new repo) |
