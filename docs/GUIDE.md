...
| `/devflow:spec <idea>` | Pre-implementation interview: rounds of option-based questions on real forks only (scope/non-goals, edge behavior, data contracts, integration, quality bar), contentious calls argued with trade-offs, an explicit "unless you say otherwise" assumption ledger → an approved spec file in `specs_dir`. Never implements | The feature exists as a picture in your head and the gap between "как я это представлял" and what gets built is expensive. Output feeds `task` (small) or `ralph-plan` (large) |
...
| The requirements live in your head, not in a ticket | `/devflow:spec` first — agree on the picture, then `task` / `ralph-plan` |
