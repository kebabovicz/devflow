...
| A feature — or a whole new project — is still a picture in your head | `/devflow:discuss <idea>` — a thorough interview (option-based questions on real forks, "I don't know" answered with proposed options, contentious calls argued, assumptions confirmed) → an approved spec/concept that `task` / `ralph-plan` consume |
