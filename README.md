...
| A feature — or a whole new project — is still a picture in your head | `/devflow:discuss <idea>` — a thorough interview: menus for enumerable forks, free-text discussion that digs into answers, "I don't know" answered with proposed options, contentious calls argued, assumptions confirmed → an approved spec/concept that `task` / `ralph-plan` consume |
