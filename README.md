...
| Anything past a one-line fix — the work needs designing, not just doing | `/devflow:map <goal>` — a decision map that carries the idea through recon, requirements, concept, spec, **detailed design**, and slicing into tickets. A shape that is expensive to change gets designed three ways in parallel before one is picked. Decisions live in files that outlive the session, so the implementing agent has nothing left to invent |
...
| Something is broken, throwing, or slow, and the cause is not obvious | `/devflow:diagnose <symptom>` — no hypotheses until one command already goes red on this bug: the loop is built from the project's own commands, then reproduce, minimise, rank falsifiable hypotheses, instrument, regression test before the fix |
| The same rejected request keeps coming back | `.out-of-scope/` — one file per concept with the reason and the prior requests; `issue`, `discuss`, and the map's charting stage check it by concept before re-litigating a settled no |
...
## The engine (`bin/devflow`)

The skills above are text a model reads and follows, which means they hold as well as the model remembers them. The commands below are the part that does not forget — they claim, refuse and record, and they answer machines rather than people: a return code first, JSON second. Not on PATH; run it as `bin/devflow` in the plugin root.

| Command | What it does |
|---|---|
| `status [--json] [path …]` | State of each project for a tree view: maps, tickets by status, contracts, open questions, live sessions. Reads only |
| `ticket take <id>` / `release <id>` | Claim a ticket, or drop the claim. The claim is a lock directory created with `mkdir`, so of several sessions asking at once exactly one wins |
...
Return codes are fixed across every subcommand: 0 done · 1 bad invocation · 2 environment not ready · 3 a gate refused · 4 someone else holds it.

## Always-on protection (hooks)

- **guard** — blocks destruction of docker volumes (`compose down -v`, `compose rm -v`, `volume rm/prune`, `system prune --volumes`; both `docker compose` and `docker-compose`); the three ways history lands on the base branch: commits on it, pushes to it (`push origin <base>`, `HEAD:<base>`, refspec deletion), and local merges/cherry-picks into it (`git pull` on base stays allowed — that's the sanctioned update); destruction of uncommitted work (`git reset --hard`, `git clean -f`); and the two acts that belong to the user rather than to a session — approving a ticket's contract and accepting its finished work;
- **ticket-guard** — a code change belongs to a ticket: an `Edit`, `Write` or `NotebookEdit` is refused while no ticket is in progress. Narrow on purpose — the map must name the branch checked out here in backticks and that effort must still have live tickets, so a project with no map for this branch, or one whose tickets are all finished, never hears from it. It never guards the map or the manifest (that is how a ticket gets taken) and never arms on the base branch. Separately, and everywhere, it refuses a hand-typed change to a ticket's `Status:`, `Contract:`, `Reported:` or `Accepted:` line — those move with commands, so that what the board shows is what actually happened. The rest of a ticket stays free to edit. Bypass: start the session as `DEVFLOW_ALLOW=1 claude` — though the honest way out is to take a ticket. A background session cannot be disarmed that way (`claude --bg` does not pass the caller's environment through) and does not need to be: it reaches work through `session start`, which takes the ticket first;
