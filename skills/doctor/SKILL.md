...
- Plugin files: `guard.sh`, `ralph-stop-gate.sh`, `init-reminder.sh`, `session-digest.sh` exist and are executable in `${CLAUDE_SKILL_DIR}/../../hooks/` (two levels up from this skill's directory = the plugin root).
...
- **Stop-gate self-test** — covered by the regression suite above; only if the suite is missing, probe manually with a disposable sandbox: `mktemp -d` with `git init`, a minimal `.devflow/project.yml` (`ralph.todo` set) and an uncommitted TODO file → pipe `{"cwd":"<sandbox>","stop_hook_active":false}` → expect exit 2 (dirty blocks); commit the TODO → expect exit 0; `"stop_hook_active":true` → always exit 0 (loop protection). Clean up the sandbox.
