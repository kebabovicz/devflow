...
- **Allowlisted tracker mutations**: scan both settings files' allowlists for outward-facing tracker mutation tools (issue create/edit/transition/comment/link — e.g. `mcp__atlassian__createJiraIssue`) → ! each: an allowlisted mutation skips the permission prompt that deterministically enforces "show the draft, ask first" (text protocols drift in long sessions; the prompt doesn't). Fix: remove from allowlist. Read-only tracker tools (get/search) are fine to allowlist.
