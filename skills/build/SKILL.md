...
4. **Verify the claim yourself**: read the diff (`git diff`), check it against the ticket's acceptance checklist and the design, re-run the checks / `test.run`. The subagent's report is a claim, not a verification. Not green → fix (or send back) until green, or set `Status: blocked` with what you learned.
