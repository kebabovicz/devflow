...
- **Loop end = handoff**: when the run finishes — TODO fully checked, or stopped on blockers — offer a review handoff for the whole task branch (format per `${CLAUDE_SKILL_DIR}/../handoff/SKILL.md`, scope: branch vs `git.base_branch`). An autonomous run is exactly the "work done while the user wasn't watching" case the handoff exists for; the TODO's `⚠ judgment call` notes feed its 🔴 section verbatim.
