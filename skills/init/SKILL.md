...
7. **CLAUDE.md section**: append (or propose) a short "devflow" section — where the manifest lives, definition-of-done rule: *changed a flow → update the mapped doc in the same change*. If the project has no CLAUDE.md at all, recommend the built-in `/init` to generate one first — devflow *consumes* project rules (review rubric, coding patterns); it does not author them. Same layer, review rules: if a `REVIEW.md` exists at the repo root, note that both the built-in code-review and devflow's review skills honor it; if the team wants custom review rules (severity, skip paths, repo-specific checks), REVIEW.md is the canonical place — mention it, don't author it.
