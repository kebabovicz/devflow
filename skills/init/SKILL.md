...
**Deferred gaps must survive init**: if the user postpones a Tier-2 plan ("not now"), record each gap as an unchecked item in `ralph.todo` (one line: the gap + the planned fix) before finishing. INIT.md dies at the end of init — a gap plan that lived only there is lost, which is exactly the knowledge loss the progress file exists to prevent.
