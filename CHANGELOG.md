...
- **A claim is now atomic instead of conventional.** The skills said to set `Status: in progress` before touching code, and two sessions reading the same file both saw it open and both wrote. `ticket take` claims through a lock directory created with `mkdir`, which either creates or fails with no window in between; eight concurrent takes on one ticket produce exactly one winner, and that is in the suite. `release` ships with it and deliberately leaves `Status` alone — the lock says who holds the work, the status says what state the work is in, and releasing after a dead session must not rewrite the second.
- **Every ticket carries a contract, and work does not start until a person agrees to it.** Not a new section: *What it delivers* and *Acceptance* already say what will be true when the ticket is finished and how anyone tells. The new `Contract:` header records the only missing fact — whether someone read them and said yes. `ticket take` refuses a `draft`, and refuses a ticket that fills neither section, which is the case the gate exists for: nobody could say when that one was done. Tickets cut before the header existed read as `legacy` and stay takeable — they were reviewed when they were sliced, and a gate that refused every existing ticket would be a migration, not a rule.
- **A question is filed with a command instead of a hand-written bullet.** A session that hits a fork had two bad options — guess, or stop and wait. `question add` gives it the third: file the question into the ticket it concerns, let that ticket block, move on to work that does not depend on the answer. Asking records the status the ticket had and answering restores it, so a session interrupted mid-work resumes where it was rather than re-entering the queue. Settled questions stay in the file with their state on the heading: the trail of what was decided is the reason to keep them.
- **`session start` makes the claim and the session one act.** It runs every gate `take` enforces, then the project's `sessions.limit`, then starts the work with `claude --bg` named after the ticket and pointed at `/devflow:task`. The engine keeps no process of its own — `claude agents` lists the session, `attach` opens it, `logs` prints it, `stop` ends it — and writes the session id into the claim, which is what lets `status` say which session is doing which ticket. `sessions.worktree: true` gives each session its own worktree; maps are never committed, so an isolated session is handed the absolute path to its ticket and still reads the one map.
- **Two new gates.** `ticket-guard.sh` refuses an `Edit`, `Write` or `NotebookEdit` when no ticket is in progress — a change that belongs to no ticket has no contract to be reviewed against and no claim keeping a second session out of the same tree. It arms narrowly: the map must name the branch checked out here in backticks, that effort must still have live tickets, and it never guards the map or the manifest, which are how a ticket gets taken in the first place. It never arms on the base branch. Guard 6 in `guard.sh` blocks `devflow contract approve` from a session — approving is the user's act, and the `DEVFLOW_ALLOW=1` prefix is what an explicit yes looks like at this layer.
- **Rules are armed by the state of the work, never by which window started the session.** A gate that applied only to sessions launched from a UI would leave every ordinary terminal unchecked, and then a ticket reaches `done` with nothing having been checked at all. The honest limits are in each hook file as usual: a write done through Bash is not an `Edit` call and is not seen, an arbitrary wrapper can smuggle a command past a regex, and a map that names another branch as the one it was cut from arms the gate for that branch too. These raise the floor against forgetting; they are not a sandbox.
- **`lib/devflow-lib.sh`** holds what the CLI and the hooks must agree on — where the map lives, what a status means, how a header is rewritten in place — because two copies of that logic drift the day someone runs the CLI from a worktree and the hook from the main checkout. Status synonyms are read and never written: `todo` and `awaiting` are understood, the canonical word is what goes back into the file.
- 195 new regression cases across five suites, including an eight-process race on one ticket and a stub CLI that records its arguments so the dispatch is asserted without starting a real session. `tests/run.sh` — 337 cases.
